...
/*
  ...
  */
class Sticky {
    constructor(node, customOffset = 0) {
        if (!(node instanceof HTMLElement))
            throw new Error('First argument must be HTMLElement');
        if (stickies.some(sticky => sticky._node === node))
            throw new Error('Polyficky is already applied to this node');
...
const Polyficky = {
    ...
    addOne(element, customOffset) {
        ...
        // Check if Polyficky is already applied to the node
    ...
    add(elementList) {
        ...
        for (let i = 0; i < nodeList.length; i++) {
            ...
            // If Polyficky is already applied to the node
            // add existing sticky
...
function init() {
    ...
    window.addEventListener('resize', Polyficky.refreshAll);
    window.addEventListener('orientationchange', Polyficky.refreshAll);
